...
# 1. Extract audio studio JSX
studio_start = dash_code.find('{/* PHASE 3: VOICE NOTE RECORDING & TIMELINE PUNCH-IN OVERWRITE */}')
studio_end = dash_code.find('{/* Modal Footer Controls */}')
studio_jsx_raw = dash_code[studio_start:studio_end]

# We want to remove the trailing `</div>` that closes the modal body, 
# so we stop right at the last `)}` of Phase 3
last_brace = studio_jsx_raw.rfind(')}')
studio_jsx = studio_jsx_raw[:last_brace+2]

# Remove the {activePhase === 3 && ( condition since we want to always show it
# (Wait, actually we can just hardcode activePhase to 3 in our new component state! That's easier.)

# 2. Extract state variables
state_start = dash_code.find('// Audio / Voice note states')
state_end = dash_code.find('// Fetch products')
audio_state = dash_code[state_start:state_end]

# 3. Extract interfaces and helpers (AudioTrackClip, TimelineHistoryStep, audioBufferToWavBlob)
helpers_start = dash_code.find('interface AudioTrackClip {')
helpers_end = dash_code.find('const WhatsAppDashboard = () =>')
helpers_code = dash_code[helpers_start:helpers_end]

# 4. Extract audio functions
func_start_token = '// Voice Note handling (Record, Pause, Resume, Stop & Web Audio Timeline Punch-In Overwrite)'
func_start = dash_code.find(func_start_token)
func_end = dash_code.find('// Reset Form')
audio_functions = dash_code[func_start:func_end]
...
print("Rebuilt with exact boundaries and no syntax errors!")
