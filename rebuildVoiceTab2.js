...
const jsxStart = dashCode.indexOf('<div className="space-y-6">', dashCode.indexOf('activePhase === 3'));
const jsxEnd = dashCode.indexOf('{/* Alternative Audio Upload */}', jsxStart);

// We grab up to the div that closes the space-y-6 wrapper.
// Actually, {/* Alternative Audio Upload */} is INSIDE space-y-6.
// Let's just grab the whole space-y-6 div.
const endOfSpaceY6 = dashCode.indexOf('</div>', jsxEnd) + 6; 
// Wait, there are multiple divs closing. Let's just grab the exact block by splitting lines.

const dashLines = dashCode.split(/\\r?\\n/);
const studioLines = dashLines.slice(2067, 2515); // This corresponds to 2068 to 2515 (0-indexed is 2067 to 2514)
// The first line is: {activePhase === 3 && (
// The last line is: )}
studioLines.shift(); // remove {activePhase === 3 && (
studioLines.pop(); // remove )}

const audioStudioJsx = studioLines.join('\\n');
...
console.log('VoiceAssetsTab.tsx rebuilt with CapCut Editor properly extracted!');
